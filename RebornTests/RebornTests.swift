...
class RebornTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        user = User(name: "测试用户", gender: .male, avatar: #imageLiteral(resourceName: "Test"), energy: 2, items: [Item]())
        user.items.append(Item(ID: 1, name: "测试1", days: 100, frequency: EveryDay(), creationDate: CustomDate.current, type: .persisting, icon: Icon.defaultIcon1, notificationTimes: [CustomTime]()))
    ...
    func testCustomTime() {
        var time1 = CustomTime(hour: 24, minute: 0, second: 0, oneTenthSecond: 0)
        var time2 = CustomTime(hour: 24, minute: 0, second: 0, oneTenthSecond: 0)
        XCTAssertTrue(time2 == time1)
        
        time1 = CustomTime(hour: 25, minute: 1, second: 1, oneTenthSecond: 1)
        time2 = CustomTime(hour: 25, minute: 1, second: 1, oneTenthSecond: 0)
        XCTAssertTrue(time1 > time2)
        XCTAssertTrue(time2 < time1)
        
        time1 = CustomTime(hour: 25, minute: 1, second: 2, oneTenthSecond: 0)
        time2 = CustomTime(hour: 25, minute: 1, second: 1, oneTenthSecond: 0)
        XCTAssertTrue(time1 > time2)
        XCTAssertTrue(time2 < time1)
        
        time1 = CustomTime(hour: 25, minute: 2, second: 1, oneTenthSecond: 0)
        time2 = CustomTime(hour: 25, minute: 1, second: 1, oneTenthSecond: 0)
        XCTAssertTrue(time1 > time2)
        XCTAssertTrue(time2 < time1)
        
        time1 = CustomTime(hour: 26, minute: 1, second: 1, oneTenthSecond: 0)
        time2 = CustomTime(hour: 25, minute: 1, second: 1, oneTenthSecond: 0)
        XCTAssertTrue(time1 > time2)
        XCTAssertTrue(time2 < time1)
        
        time1 = CustomTime(hour: -1, minute: -1, second: -1, oneTenthSecond: 0)
        time2 = CustomTime(hour: -1, minute: -1, second: -1, oneTenthSecond: 0)
        XCTAssertTrue(time1 == time2)
        
        time1 = CustomTime(hour: -1, minute: -1, second: -1, oneTenthSecond: 0)
        time2 = CustomTime(hour: -2, minute: -1, second: -1, oneTenthSecond: 0)
        XCTAssertTrue(time1 > time2)
        XCTAssertTrue(time2 < time1)
        
        time1 = CustomTime(hour: -2, minute: -1, second: -1, oneTenthSecond: 0)
        time2 = CustomTime(hour: -2, minute: -1, second: -1, oneTenthSecond: -1)
        XCTAssertTrue(time1 > time2)
        XCTAssertTrue(time2 < time1)
        
        time1 = CustomTime(hour: -2, minute: -1, second: -1, oneTenthSecond: 0)
        time2 = CustomTime(hour: -2, minute: -1, second: -1, oneTenthSecond: -2)
        XCTAssertTrue(time1 > time2)
        XCTAssertTrue(time2 < time1)
    }
