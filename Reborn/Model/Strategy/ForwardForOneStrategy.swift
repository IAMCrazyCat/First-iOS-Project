...
class ForwardForOneStrategy: PagesBehaviorStrategy {
    ...
    override func updateCalendarPagesColor() {
        ...
        if 1 <= self.viewController.calendarPages.count - 1 {
    ...
    override func addNewCalendarPage() {
        ...
        newCalendarPage.frame = viewController.calendarPages.first?.frame ?? { print("Found nil, location: \(self) Line: \(#line)"); return CGRect.zero }()// new calendar page frame equals to the first page
        newCalendarPage.frame.origin.y = viewController.calendarPages.first?.frame.origin.y ?? { print("Found nil, location: \(self) Line: \(#line)"); return 0 }() - self.setting.newCalendarPageCordiateYDifference * CGFloat(self.viewController.calendarPages.count) // move up the new calendar page to its proper position Y
        ...
        self.viewController.view.insertSubview(newCalendarPage, belowSubview: self.viewController.calendarPages.last ?? self.viewController.view)
    ...
    override func updateTempCalendarPage() {
        ...
        let calendarViewController = self.viewController.calendarViewController {
            ...
            for index in 0 ... self.viewController.calendarPages.count - 1 {
                let builder = TimeMachineCalendarPageBuilder(interactableCalendarView: calendarView, calendarViewController: calendarViewController, monthDifference: -index + 1)
